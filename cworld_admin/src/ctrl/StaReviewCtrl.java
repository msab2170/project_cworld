...
@WebServlet("/sta_review")
public class StaReviewCtrl extends HttpServlet {
	private static final long serialVersionUID = 1L;
    public StaReviewCtrl() { super(); }
    ...
    protected void doProcess(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		String review = staHomeSvc.getReview(startDate, endDate);
		request.setAttribute("review", review);
		
		String amount = staHomeSvc.getAmount(startDate, endDate);
		request.setAttribute("amount", amount);
		request.setAttribute("sch", sch);
		RequestDispatcher dispatcher = request.getRequestDispatcher("statistics/sta_review.jsp");
